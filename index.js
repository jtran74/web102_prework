...
contributionsCard.innerHTML = `
   <p> $${totalContribution.toLocaleString("en")} </p>
`;


// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const pledgeAmount = GAMES_JSON.reduce( (total, games) => {
    return total + games.pledged;
}, 0); 
// set inner HTML using template literal
raisedCard.innerHTML = `
   <p> $${pledgeAmount.toLocaleString("en")} </p>
`;

// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");
const goalAmount = GAMES_JSON.length
gamesCard.innerHTML = `
...
let unfundedGames = `
    A total of ${pledgeAmount.toLocaleString("en")} has been raised for 11 games. Currently, ${numUnfundedGames.length} games remain 
...
const nextGame = document.createElement("p")
nextGame.innerHTML = `
    ${secondGame.name}
`
secondGameContainer.appendChild(nextGame)
